  ...
  return (
    <header style={{ background: COLORS.surface, borderBottom: `1px solid ${COLORS.border}`, position: "sticky", top: 0, zIndex: 50 }}>
      <div style={{ maxWidth: 1180, margin: "0 auto", padding: "12px 20px", display: "flex", alignItems: "center", gap: 18, flexWrap: "wrap" }}>
        <a href="/" style={{ color: COLORS.navy, fontWeight: 800, textDecoration: "none", fontSize: 19, flexShrink: 0, marginRight: "auto" }}>
          PACK OF FIVE
        </a>
        ...
        <a href={accountHref} style={{ textDecoration: "none", background: COLORS.navy, color: "#fff", borderRadius: 8, padding: "9px 14px", fontSize: 14, fontWeight: 700, flexShrink: 0, whiteSpace: "nowrap" }}>
          {accountLabel}
        </a>
      </div>
    </header>
    ...
    <div style={{ minHeight: "100vh", background: COLORS.background }}>
      <div style={{ display: "grid", gridTemplateColumns: "240px minmax(0, 1fr)", minHeight: "100vh" }}>
        <aside style={{ background: COLORS.navy, color: "#fff", padding: "24px 18px" }}>
          ...
          <div style={{ fontSize: 12, opacity: 0.72, marginTop: 3, marginBottom: 28, letterSpacing: "0.08em" }}>RESCUE MANAGER</div>
          ...
          <div style={{ borderTop: "1px solid rgba(255,255,255,.16)", marginTop: 28, paddingTop: 18 }}>
            ...
            <div style={{ fontSize: 12, lineHeight: 1.4, color: "rgba(255,255,255,.72)", overflowWrap: "anywhere", marginBottom: 14 }}>{user.email}</div>
...
function ManagerLink({ href, children }: { href: string; children: ReactNode }) {
  ...
  return (
    <a href={href} style={{ display: "block", padding: "10px 12px", borderRadius: 7, color: "#fff", textDecoration: "none", marginBottom: 4, background: active ? "rgba(255,255,255,.13)" : "transparent", fontWeight: active ? 700 : 500, fontSize: 14 }}>
      {children}
    </a>
  );
}
