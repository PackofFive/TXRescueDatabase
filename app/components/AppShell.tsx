...
export default function AppShell({ children, user }: { children: ReactNode; user: ShellUser }) {
  const pathname = usePathname();
  const isAdminArea = pathname === "/admin" || pathname.startsWith("/admin/");
  const isManagerArea =
    pathname === "/portal" || pathname.startsWith("/portal/") ||
    pathname === "/animals" || pathname.startsWith("/animals/");

  if (isAdminArea) return <AdminShell>{children}</AdminShell>;

  if (isManagerArea && user?.role === "org" && user.status === "approved") {
  ...
  return (
    <>
      ...
      <main style={{ padding: "28px 24px", maxWidth: 1180, margin: "0 auto" }}>{children}</main>
    </>
  );
}
...
function PublicHeader({ user }: { user: ShellUser }) {
  ...
  const accountLabel = user?.role === "admin" ? "Admin" : user ? "Rescue Manager" : "Sign In";
  return (
    <header style={{ background: COLORS.surface, borderBottom: `1px solid ${COLORS.border}`, position: "sticky", top: 0, zIndex: 50 }}>
      <div style={{ maxWidth: 1180, margin: "0 auto", padding: "14px 24px", display: "flex", alignItems: "center", justifyContent: "space-between", gap: 24, flexWrap: "wrap" }}>
        <a href="/" style={{ color: COLORS.navy, fontWeight: 800, textDecoration: "none", fontSize: 19 }}>PACK OF FIVE</a>
        <nav aria-label="Public navigation" style={{ display: "flex", alignItems: "center", gap: 20, flexWrap: "wrap" }}>
          ...
          <a href="/support" style={{ ...publicLinkStyle, color: COLORS.coral, fontWeight: 700 }}>Support Pack of Five</a>
          <a href={accountHref} style={{ textDecoration: "none", background: COLORS.navy, color: "#fff", borderRadius: 8, padding: "9px 14px", fontSize: 14, fontWeight: 700 }}>{accountLabel}</a>
        </nav>
      </div>
    </header>
  );
      ...
      <div style={{ display: "grid", gridTemplateColumns: "240px minmax(0, 1fr)", minHeight: "100vh" }}>
        <aside style={{ background: COLORS.navy, color: "#fff", padding: "24px 18px" }}>
          ...
          <nav aria-label="Rescue Manager navigation">
            ...
            {/* Tags & Transfers, Fosters, Tasks, Reports and Files are added as routes are built. */}
          </nav>
          <div style={{ borderTop: "1px solid rgba(255,255,255,.16)", marginTop: 28, paddingTop: 18 }}>
            <a href="/" style={{ display: "block", color: "#fff", textDecoration: "none", fontSize: 14, marginBottom: 12 }}>← Rescue Network</a>
            <a href="/pets" style={{ display: "block", color: "#fff", textDecoration: "none", fontSize: 14, marginBottom: 12 }}>Owner Portal</a>
            <div style={{ fontSize: 12, lineHeight: 1.4, color: "rgba(255,255,255,.72)", overflowWrap: "anywhere" }}>{user.email}</div>
          </div>
        </aside>
        <div style={{ minWidth: 0 }}>
          <header style={{ background: COLORS.surface, borderBottom: `1px solid ${COLORS.border}`, padding: "16px 28px" }}>
            <div style={{ maxWidth: 1120, margin: "0 auto" }}>
              ...
              <div style={{ fontSize: 12, color: COLORS.muted }}>Private organization workspace</div>
            </div>
          </header>
    ...
    <div style={{ minHeight: "100vh", background: "#F7F7F8" }}>
      <header style={{ background: "#111827", color: "#fff", padding: "16px 24px" }}>
        <div style={{ maxWidth: 1180, margin: "0 auto", display: "flex", justifyContent: "space-between", alignItems: "center", gap: 20 }}>
          ...
          <nav style={{ display: "flex", gap: 18 }}><a href="/admin" style={darkLinkStyle}>Admin Dashboard</a><a href="/" style={darkLinkStyle}>Rescue Network</a></nav>
        </div>
      </header>
...
const publicLinkStyle = { textDecoration: "none", color: COLORS.text, fontSize: 14 } as const;
