...
export default function AdminAddOrganizationPage() {
  ...
  async function submit(e: React.FormEvent) {
    ...
    try {
      ...
      setStatus(`${data.organization.name} was created.`);
    } catch (err) {
  ...
  return (
    <section style={{ maxWidth: 760 }}>
      ...
      <p style={{ color: "#6B6862", lineHeight: 1.6 }}>
        Create a rescue, shelter, or other organization directly in the Pack of Five directory.
      </p>
      ...
      <form onSubmit={submit} style={{ display: "grid", gap: 14 }}>
        ...
        <div style={twoCol}>
          <Field label="State / Region" value={form.region} onChange={(v) => update("region", v)} />
        ...
        <label style={labelStyle}>
          ...
          <select
            ...
            onChange={(e) => update("resourceStatus", e.target.value)}
            style={inputStyle}
          >
        ...
        <div style={{ marginTop: 18, padding: 12, border: "1px solid #E7E5E1", borderRadius: 8, background: "#fff" }}>
          ...
          {createdOrgId && (
            <div style={{ marginTop: 8 }}>
              <a href={`/admin/orgs/${createdOrgId}`}>Open organization editor →</a>
            </div>
          )}
        </div>
      )}
