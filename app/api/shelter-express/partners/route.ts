...
export async function PATCH(request:NextRequest){
  try{
    ...
    const relationshipStatus=typeof body?.relationshipStatus==="string"?body.relationshipStatus:"considering";
    ...
    if(!rescueOrgId||!["considering","vetting","approved","preferred","on_hold","do_not_use"].includes(relationshipStatus))return NextResponse.json({error:"Choose a valid partner and relationship status."},{status:400});
    if(["on_hold","do_not_use"].includes(relationshipStatus)&&!privateNotes)return NextResponse.json({error:"Add a private note explaining why this partner is on hold or should not be used."},{status:400});
