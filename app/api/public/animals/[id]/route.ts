...
export async function GET(_request:Request,{params}:{params:Promise<{id:string}>}){try{const {id}=await params;const rows=await sql`
 ...
 organization.id as organization_id,organization.name as organization_name,organization.org_type as organization_type,organization.city as organization_city,
 ...
 exists(
   select 1 from animal_help_offers offer
   where offer.animal_id=animal.id and offer.offer_type='tag_request'
   and offer.status='accepted' and offer.transfer_completed_at is null
 ) as tag_pending
 ...
 return NextResponse.json({animal:{id:row.id,name:row.name,species:row.species,breed_or_type:row.breed_or_type,birth_date:row.birth_date,sex:row.sex,weight_lbs:row.weight_lbs,public_summary:row.public_summary,public_need:row.public_need,external_listing_url:row.external_listing_url,outcome_status:row.outcome_status,outcome_date:row.outcome_date,public_outcome_message:row.public_outcome_message,show_on_success_wall:Boolean(row.show_on_success_wall),active_help_offer_count:Number(row.active_help_offer_count??0),tag_pending:Boolean(row.tag_pending),photo:row.primary_photo_document_id?{id:String(row.primary_photo_document_id),url:`/api/public/animals/${encodeURIComponent(id)}/photo`,source:"animal_document",visibility:"public"}:null,organization:{id:row.organization_id,name:row.organization_name,type:row.organization_type,city:row.organization_city,state:row.organization_state,website:row.organization_website,email:row.organization_email,phone:row.organization_phone}}},{headers:{"Cache-Control":"public, max-age=60, stale-while-revalidate=300"}});
