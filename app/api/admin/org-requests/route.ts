...
export async function GET() {
  try {
    ...
    return NextResponse.json(
      { error: "Couldn't load organization requests." },
      { status: 500 }
    );
  }
}
...
export async function POST(req: NextRequest) {
  try {
    ...
    if (duplicate[0]) {
      return NextResponse.json(
        {
          error: "A matching organization already exists.",
    ...
    const orgRows = await sql`
      insert into organizations
        (name, city, county, region, website, email, phone, resource_status)
      values
        (
          ...
          ${request.county || null},
          ${request.state || null},
          ${request.website || request.social_url || null},
          ${request.contact_email || null},
          ${request.contact_phone || null},
          'Verification Needed'
        )
      returning id, name, city, county, region, website, email, phone, resource_status
    `;
    ...
    try {
      ...
      console.error("Approved org request but audit log failed:", auditErr);
    }
    ...
    return NextResponse.json(
      { error: "Couldn't review this organization request." },
      { status: 500 }
    );
  }
}
