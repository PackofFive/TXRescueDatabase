      ...
      <article key={offer.id} style={card}>
        ...
        {offer.transfer_completed_at?<div style={completedTransfer}><strong>Custody transferred to {offer.receiving_organization_name||"the receiving rescue"}.</strong><span>Confirmed {new Date(offer.transfer_completed_at).toLocaleString()}{offer.transfer_confirmed_by_email?` by ${offer.transfer_confirmed_by_email}`:""}.</span><span>This permanent record is now read-only. The receiving rescue manages the animal&apos;s current record.</span></div>:offer.placed_with_another_rescue?<div style={completedTransfer}><strong>This request closed when the animal transferred to another rescue.</strong><span>The request remains available as a permanent private record and cannot be reopened.</span></div>:<div style={workflow}>
          ...
          <div style={workflowActions}>
            ...
            {offer.status === "accepted" && offer.offer_type !== "tag_request" ? <ActionButton label="Close completed offer" onClick={() => void update(offer.id, "closed")} disabled={workingId === offer.id}/> : null}
            {offer.status === "accepted" && offer.offer_type === "tag_request" ? <ActionButton label="Withdraw approval" onClick={() => void update(offer.id, "reviewing")} disabled={workingId === offer.id} danger/> : null}
...
function helpForStatus(status: string) { const help: Record<string, string> = { new: "New offer—review the details and contact the person before making a decision.", reviewing: "Your shelter is reviewing this offer.", contacted: "Your shelter has contacted the person and is awaiting or coordinating next steps.", accepted: "Approved and awaiting the receiving rescue's confirmation after physical custody changes. The shelter remains the source of record until then.", declined: "Your shelter declined this offer. The record remains available in Closed.", closed: "Work on this offer is complete. The record remains available in Closed." }; return help[status] || "Review this offer."; }
